"""
Rutas para procesamiento de Etiqueta de Envío ENVIAME
"""
...
from models.document_type_models import EtiquetaEnviameRequest
from services.document_processors.etiqueta_enviame_processor import EtiquetaEnviameProcessor
...
# Crear router
router = APIRouter(prefix="/api/v1", tags=["etiqueta-enviame"])

# Inicializar procesador de forma lazy
_enviame_processor = None

def get_enviame_processor():
    """Obtiene la instancia del procesador (lazy initialization)"""
    global _enviame_processor
    if _enviame_processor is None:
        _enviame_processor = EtiquetaEnviameProcessor()
    return _enviame_processor


@router.post("/etiqueta_enviame", response_model=Dict[str, Any])
async def process_etiqueta_enviame(
    request: EtiquetaEnviameRequest,
    background_tasks: BackgroundTasks
) -> Dict[str, Any]:
    """
    Endpoint para procesar una Etiqueta de Envío ENVIAME
    ...
    with document_logging_context(doc_id=doc_id, provided_classification="Etiqueta de Envío ENVIAME", stage="ingest"):
        try:
            logger.info(f"Recibida solicitud de procesamiento para Etiqueta ENVIAME {doc_id}")
            ...
            set_stage("queued")
            background_tasks.add_task(process_document_background, document_data, "enviame")
            
            return {
                "message": "Etiqueta ENVIAME recibida para procesamiento",
                "document_id": doc_id,
                "status": "PROCESSING",
            }
...
async def process_document_background(document_data: Dict[str, Any], processor_type: str):
    ...
    with document_logging_context(
        doc_id=document_id,
        provided_classification="Etiqueta de Envío ENVIAME",
        ...
        try:
            ...
            processor = get_enviame_processor()
